...
public class RomanToInteger {
	...
	public static void main(String[] args) {
		...
		System.out.println("Output: " + test.romanToInt("LVIII"));
	}

	public int returnInt(char roman) {
	...
	public int romanToInt(String s) {
		...
		for (int i = 0; i < s.length(); i++) {
			int current = returnInt(s.charAt(i));

			if (i == s.length() - 1)
				// last letter(number) should just be added to the result
				result += current;
			else {
				int next = returnInt(s.charAt(i + 1));
				// we are subtracting the current letter(number) when the next one is greater
				if (next > current)
					result -= current;
				else
					// otherwise we will simply add the current letter(number)
					result += current;
			}
		}
